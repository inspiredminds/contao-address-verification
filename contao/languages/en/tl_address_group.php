<?php

declare(strict_types=1);

/*
 * This file is part of the Contao Address Verification extension.
 *
 * (c) inspiredminds
 *
 * @license LGPL-3.0-or-later
 */

$GLOBALS['TL_LANG']['tl_address_group']['new'] = ['Create new address group', 'Create a new address group'];
$GLOBALS['TL_LANG']['tl_address_group']['edit'] = ['Edit address group ID %s', 'Edit address group ID %s'];
$GLOBALS['TL_LANG']['tl_address_group']['address_group_legend'] = 'Address group';
$GLOBALS['TL_LANG']['tl_address_group']['address_group_content_legend'] = 'Verified address content';
$GLOBALS['TL_LANG']['tl_address_group']['name'] = ['Name', 'Name of the address group.'];
